...
The dataset under review, named %DATASET_NAME%, is %DATASET_VOLUME% in size and contains %DATASET_RECORD_COUNT% occurence records with %DATASET_TAXON_COUNT% unique taxon names (e.g., %DATASET_TAXON_MOST_FREQUENT%).
...
| name | value |
| --- | --- | 
| **Number of unique taxonNames:** | %DATASET_TAXON_COUNT% | 
